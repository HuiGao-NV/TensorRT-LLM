...
Look for `slurm-cluster-*.md` files in `skills/slurm-cluster-info/references/`.
Each file configures one cluster using the naming convention
`slurm-cluster-<name>.md` (e.g., `slurm-cluster-lyris.md`).

**HARD GATE: If no `slurm-cluster-*.md` file exists, you MUST create one before
doing ANYTHING else. No SSH commands. No `ssh -O check`. No `grep ~/.ssh/config`.
No guessing hostnames. No inferring from context. STOP and create the config.**

**Discovery:**
1. If no config files found → create one (see below)
2. If exactly one found → use it
3. If multiple found → match the cluster name from the user's request. If no
   cluster is mentioned, list the available clusters and ask which one to use.

To create a config:
1. Read the template from [references/slurm-cluster-template.md](references/slurm-cluster-template.md)
2. Ask the user for the required fields. At minimum you need:
   - `cluster`: cluster name (used in the filename and reference lookup)
   - `ssh_host`: what they type after `ssh` to connect (e.g., `lyris`,
     `wkong-mfa@login-lyris`)
   - `user`: their SLURM username (for `squeue -u`)
   - `remote_cwd`: working directory on the cluster
   - `account`: SLURM account
   - `subproject`: for job naming
   - `partition`: default partition
   - `container_image`: container image URI
   - `mounts`: host:container mount paths
3. Create `slurm-cluster-<name>.md` in `skills/slurm-cluster-info/references/`
   (where `<name>` matches the `cluster` field)
4. The user can refine it further after creation

Once the file exists, parse the YAML frontmatter to extract: `ssh_host`, `user`,
`remote_cwd`, `account`, `subproject`, `partition`, `container_image`, `mounts`,
and optionally `gpus_per_node` (required on shared/QOS clusters) and `packages`
...
**Skip to Step 3.** Do NOT create a TRT-LLM-specific ControlMaster; reuse
...
After the user confirms, verify with `ssh -o ControlPath=/tmp/ssh-trtllm-%r@%h:%p -O check <ssh_host>`.
...
```bash
ssh <ssh_host> "tmux new-session -d -s trtllm-<id> \
...
ssh <ssh_host> "tmux kill-session -t trtllm-<id>"
```
...
- tmux sessions: `ssh <ssh_host> "tmux has-session -t trtllm-<id> 2>/dev/null && echo exists"`
...
- Kill tmux sessions: `ssh <ssh_host> "tmux kill-session -t trtllm-<id>"`
